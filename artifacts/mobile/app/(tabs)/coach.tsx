...
const AI_RESPONSES: Record<string, string> = {
  default: "I'm analyzing your financial patterns now. Based on your profile, I can see opportunities to strengthen your financial wellness. What specific area would you like to focus on today?",
  overspend: "Based on your transaction history, your impulse spending tends to spike on weekends. This is linked to emotional spending triggers — particularly stress relief. Try the 24-hour rule: wait a full day before non-essential purchases over 200 EGP. Your Money Profile suggests you benefit from structured cooling-off periods.",
  ...
  spending: "Your top 3 spending categories are: Food & Drink (28%), Shopping (22%), and Entertainment (15%). Compared to last month, shopping increased by 18% — this correlates with high-stress periods in your mood log. Reducing shopping by 30% would add 580 EGP to your savings monthly.",
};

function getAIResponse(input: string): string {
  const lower = input.toLowerCase();
  if (lower.includes("overspend") || lower.includes("spend too much")) return AI_RESPONSES.overspend;
  if (lower.includes("save") || lower.includes("car") || lower.includes("saving")) return AI_RESPONSES.save;
  if (lower.includes("debt") || lower.includes("loan")) return AI_RESPONSES.debt;
  if (lower.includes("analyz") || lower.includes("spending")) return AI_RESPONSES.spending;
      ...
      >
          ...
          renderItem={({ item }) =>
            item.role === "user" ? (
              ...
              <View style={styles.aiBubbleWrap}>
                <View style={[styles.bubble, styles.aiBubble, { backgroundColor: colors.card }]}>
                  <Text style={[styles.aiText, { color: colors.foreground }]}>{item.text}</Text>
                </View>
              </View>
            )
          }
        ...
        <View style={[styles.suggestedWrap, { borderTopColor: colors.border }]}>
          ...
          >
            {SUGGESTED.map((s) => (
              <Pressable
                key={s}
                style={[styles.suggestedChip, { backgroundColor: colors.card, borderColor: colors.border }]}
                onPress={() => sendMessage(s)}
              >
                <Text style={[styles.suggestedText, { color: colors.foreground }]}>{s}</Text>
              </Pressable>
            ))}
          </ScrollView>
        </View>
        ...
        >
          ...
          >
            <LinearGradient
              colors={["#2E3192", "#92278F"]}
              style={[styles.sendBtnGrad, (!input.trim() || isTyping) && { opacity: 0.4 }]}
            >
...
const styles = StyleSheet.create({
  ...
  coachAvatarGrad: { width: 44, height: 44, borderRadius: 22, alignItems: "center", justifyContent: "center" },
  onlineDot: {
    position: "absolute", bottom: 0, right: 0,
    width: 12, height: 12, borderRadius: 6,
    backgroundColor: "#22C55E", borderWidth: 2, borderColor: "#0B1026",
  },
  coachInfo: { flex: 1 },
  coachName: { fontSize: 16, fontFamily: "Inter_600SemiBold", color: "#FFFFFF" },
  coachStatus: { fontSize: 12, fontFamily: "Inter_400Regular", color: "rgba(255,255,255,0.5)", marginTop: 2 },
  ...
  aiBubble: { borderBottomLeftRadius: 4, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 6, elevation: 1 },
  ...
  input: { flex: 1, borderRadius: 22, paddingHorizontal: 16, paddingVertical: 10, fontSize: 14, fontFamily: "Inter_400Regular", maxHeight: 100, lineHeight: 20 },
  sendBtn: {},
